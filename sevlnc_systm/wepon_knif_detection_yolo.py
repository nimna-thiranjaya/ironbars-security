...
# Model
model = torch.hub.load('ultralytics/yolov5','custom', path="wepon_knif_detection_modle_20k.pt")
    ...
    for obj in range(obiect_list.shape[0]):
        ...
        try:
            if rect_overlayer:
                ...
                #if int(clas) != 0 and int(clas) != 25 and float(confidence) > 0.4 :
                if True:
...
# cap = cv2.VideoCapture('gun side.mp4')
#
# while True:
#     r, rim = cap.read()
